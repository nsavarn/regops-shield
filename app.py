...
def health_check():
    db_ok = mongo is not None and mongo.health_check()
...
def run_shadow_simulation(payload: ClaimInput, policies: Optional[List[PolicyInput]] = None):
    try:
        ...
        # Two paths:
        # 1. Preloaded policies → vector search + direct analysis
        # 2. No policies → supervisor autonomously calls search_policies tool
        policies_list = [p.model_dump() for p in policies] if policies else None

        # Run shadow simulation via Supervisor Agent (Gemini 2.0 Flash + Native Tools)
        ...
        if remediation:
            ...
            if final_remediation:
                ...
                mongo.update_session(str(session_id), {"remediation_suggestion": final_remediation})

        logger.info(
            f"Shadow run completed for claim {payload.claim_id} | "
            f"Risk: {session.risk_level} | Tools: {session.tool_calls_made}"
        )
        ...
        return SessionResponse(
            ...
            remediation_suggestion=session.remediation_suggestion or "",
...
def get_audit_packet(session_id: str):
    ...
    try:
        # Reconstruct session object for audit generator
        from agents.supervisor import ShadowRunSession
        session = ShadowRunSession.model_validate(session_doc)
        packet = audit_generator.generate(session)
...
def vector_search_policies(query: Dict[str, str] = Field(..., description="Search text")):
    ...
    try:
        # Uses Atlas Vector Search with Google text-embedding-004
